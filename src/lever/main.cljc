(ns lever.main
  "Kotodama WASM entrypoint for the Lever clean-room actor (L4) — Clojure port.
  ...
  facts (`lever.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"lever.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lever-compat" :tier tier :entities entities} 200])
